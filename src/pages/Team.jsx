...
export default function Team({ supabase, isCommish }) {
  ...
  function renderGameLine(g) {
    const prefix = (g.home_away || "HOME") === "AWAY" ? "@ " : "vs. ";
    ...
    return (
      <div className="gameLine">
        ...
        <div className="gameMeta">
          ...
          {fr.score ? (
            <span className="scoreText">{fr.score}</span>
          ) : fr.text ? (
            <span className="muted">{fr.text}</span>
          ) : null}
        </div>
      </div>
    );
  ...
  return (
    <div className="teamPage">
      {notice ? (
        <div className="card" style={{ borderColor: "rgba(43,212,106,.30)", background: "rgba(43,212,106,.08)", marginBottom: 12 }}>
          {notice}
        </div>
      ) : null}
      {error ? (
        <div className="card" style={{ borderColor: "rgba(255,90,95,.35)", background: "rgba(255,90,95,.08)", marginBottom: 12 }}>
          {error}
        </div>
      ) : null}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      ...
                      {canEdit ? (
                        <div className="row" style={{ flex: "0 0 auto", gap: 8 }}>
                          <button className="btn small" onClick={() => startEditPlayer(p)} type="button">Edit</button>
                          <button className="btn danger small" onClick={() => deletePlayer(p.id)} type="button">Delete</button>
                        </div>
                      ) : null}
                    </div>
                  )}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      ...
                      {canEdit ? (
                        <div className="row" style={{ flex: "0 0 auto", gap: 8 }}>
                          ...
                          <button className="btn danger small" onClick={() => deleteGame(g.id)} type="button">Delete</button>
                        </div>
                      ) : null}
                    </div>
                  )}
